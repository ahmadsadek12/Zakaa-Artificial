...
export default function Calendar() {
  ...
  const fetchScheduledData = async () => {
    try {
      ...
      // Fetch reservations (gracefully handle 403 if addon not enabled)
      try {
        ...
        if (error.response?.status === 403) {
          // Reservations addon not enabled, just set empty array
          setReservations([])
        } else {
      ...
      // Fetch tables for F&B businesses (gracefully handle 403 if addon not enabled)
