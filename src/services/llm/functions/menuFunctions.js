  ...
  return [
    {
      ...
      function: {
        name: 'get_services',
        description: 'Get all available menu items, menu images, or menu PDFs. ⚠️ CRITICAL RULES: ONLY use this when customer\'s CURRENT message EXPLICITLY asks to see the menu, wants to browse items, asks "what do you have?", "show me the menu", "send menu", "menu please", or requests the menu in any format. ⚠️ IMPORTANT: If customer EXPLICITLY asks for menu, ALWAYS call this function - even if they have items in cart. Customers can browse the menu at any time. DO NOT use this: (1) for greetings ("Hello", "Hi", "Hey") - just greet back; (2) when customer is ordering items (e.g., "I want pizza" - use add_service_to_cart instead); (3) when customer is checking item availability (e.g., "do you have pepsi?" - use check_item_availability instead); (4) when customer is managing their order WITHOUT asking for menu (e.g., "I want 3 trios", "remove 3 of the 6", "fix the cart"); (5) when customer is providing delivery information WITHOUT asking for menu (e.g., "I want it delivered", "delivered", providing addresses). If menu was already shown in recent messages, you can still show it again if customer explicitly asks. This function automatically handles PDFs, images, and text-based menus based on what\'s available.',
        parameters: {
          type: 'object',
          properties: {},
          required: []
