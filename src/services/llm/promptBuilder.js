...
async function buildPrompt({ business, branch, customerPhoneNumber, message, language, messageHistory = [], isFirstMessage = false, shouldGreet = false }) {
  ...
  // Build cart summary if items exist (only show if cart has items)
  const cartSummary = cart && cart.items && cart.items.length > 0 
    ? cartManager.getCartSummary(cart)
    : null; // Don't show "empty" - just don't mention cart if it's empty
...
If customer says ANY of these: "reserve a table", "book a table", "table reservation", "I want to reserve", "reservation", "I'm reserving", "reserve for [time]" → 
  ...
  → ⚠️ CRITICAL: If create_table_reservation() returns ANY error, DO NOT call request_human_assistance(). Just tell the customer the error message and suggest trying again or choosing different options. Reservation errors are ALWAYS fixable without human assistance.
...
8. ⚠️ HUMAN ASSISTANCE: If you cannot help the customer, customer explicitly asks for a human, or the issue is too complex, use request_human_assistance() to lock the session, create a support ticket, and connect with an employee.
9. ONLY when customer wants to ORDER (not reserve), then follow the order process
...
- Step 4: ⚠️ CRITICAL - DELIVERY TYPE (ONLY IF OPEN, AND ONLY FOR ORDERS, NOT RESERVATIONS): When order items are decided AND business is OPEN, you MUST ALWAYS ask: "Would you like this delivered, for dine-in, or takeaway?" UNLESS the customer has ALREADY mentioned their preference in the current or recent message. NEVER assume delivery type - ALWAYS ask if not explicitly mentioned. DO NOT ask about delivery type if business is closed - schedule first. ⚠️ IMPORTANT: If customer is making a TABLE RESERVATION (said "reserve a table", "book a table"), do NOT ask about delivery - reservations are always dine-in. Only ask about delivery type for ORDERS (when customer is ordering food items).
...
- ⚠️ CRITICAL DISTINCTION: Table reservations are DIFFERENT from orders:
  ...
  - ⚠️ CRITICAL: When handling a RESERVATION request, IGNORE the cart completely. Do NOT mention the cart, do NOT check if cart is empty, do NOT suggest adding items to cart. Reservations are separate from orders. Only mention the cart if the customer is explicitly trying to ORDER food, not when they're making a RESERVATION.
...
- ⚠️ CRITICAL: If create_table_reservation() returns an error, DO NOT call request_human_assistance(). Simply tell the customer the error message from the function. Most reservation errors are fixable (e.g., "table already reserved", "no tables available", "technical issue") - just suggest trying a different time or table. NEVER escalate reservation errors to human assistance - they are always fixable by trying again or choosing different options.
